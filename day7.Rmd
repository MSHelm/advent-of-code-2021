---
title: "AoC 2021 - Day 7: The Treachery of Whales"
...
The first part is simple the distance to the median!
```{r}
fuel_needed <- sum(abs(x - median(x)))
print(paste("Amount of fuel needed:", fuel_needed))
```
...
### Explanation why the mean works:
...
```{r}
distances <- abs(x - floor(mean(x)))

triangle_cost <- function(d) {
  cost <- (d * (d + 1)) / 2
  return(cost)
}

costs <- triangle_cost(distances)
fuel_needed_part2 <- sum(costs)

print(paste("Total fuel needed:", fuel_needed_part2))

```

## Learnings

Sometimes very basic descriptive statistics can be very useful for optimization :)
I still dont know why I need to floor the mean, not round it!